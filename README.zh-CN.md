...
# 首次启动会自动创建 little_alphaxiv.db 并生成 backend/.env
...
| 变量 | 默认 | 用途 |
|-----|---------|---------|
| `LAX_DATABASE_URL` | `sqlite:///./little_alphaxiv.db` | SQLite 文件。Docker 下为 `sqlite:////app/data/little_alphaxiv.db`。 |
| `LAX_SECRET_KEY` | *(自动生成)* | 用于加密存储的 API key + 签名会话 cookie 的 Fernet key。首次运行自动生成并写入 `./data/.lax_secret_key`（Docker）或 `backend/.env`（本地）。**务必保密——丢失它会让所有加密 key + 会话全部失效。** |
...
| `LAX_PDF_CACHE` | `~/.little_alphaxiv/pdf_cache` | PDF 磁盘缓存目录（按内容寻址、全局、非敏感）。Docker 下为 `/app/data/pdf_cache`。 |
...
`./data/.lax_secret_key`（Docker）或 `backend/.env`（本地）。
</details>
