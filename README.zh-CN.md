...
如果想把项目交付成一个 app，而不是要求用户安装 Docker，可以构建一键运行的
Linux 便携包。构建脚本会编译 Vite 前端、复制后端与 Alembic 迁移、打入 Python
运行时，并写入 `AppRun` 启动器：
...
如果要单个可执行文件，构建自解压 runner：

```bash
packaging/linux/build-linux-run.sh
```

产物是 `dist/linux/LittleAlphaxiv-<version>-x86_64.run`，这才是适合分发的单文件
Linux app。首次启动会解压到 `~/.cache/little-alphaxiv/`，运行数据保存在
`$XDG_DATA_HOME/little-alphaxiv` 或 `~/.local/share/little-alphaxiv`。
