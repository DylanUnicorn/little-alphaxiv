...
务。`LAX_SECRET_KEY` 会在首次启动时自动生成并写入 `./data` 卷，所以**零配
置**即可启动。

```bash
git clone https://github.com/DylanUnicorn/little-alphaxiv.git
cd little-alphaxiv
docker compose up -d            # 构建并在 http://127.0.0.1:8000 启动
```
...
数据（SQLite 数据库、PDF 缓存、持久化的 secret key）都在 `./data/`。
查看日志：`docker compose logs -f little-alphaxiv`。停止：`docker compose down`。

> **无需 SMTP 的密码找回：** 默认情况下，重置链接会打印到容器日志（本地用
> 无需邮件服务器）。若要发送邮件，在根目录 `.env` 里设置 `LAX_SMTP_URL`
> ——见「配置（环境变量）」一节。
...
# 首次启动会自动创建 backend/data/little_alphaxiv.db 并生成 backend/data/.lax_secret_key
...
> **从旧版本升级？** 更新后首次启动，后端会自动把已有数据迁进统一的
> `backend/data/` 目录——SQLite DB（含你的所有账号、对话、论文、已保存的
> provider）、Fernet 密钥、密码重置链接日志都会从后端根目录搬走。无需任何
> 操作，数据原样保留（密钥复用，加密的 API key + 现有会话继续可用）。
...
全部可选——默认值即可用于 localhost。Docker 下在根目录 `.env`（从
`.env.docker.example` 拷贝）设置；本地开发拷贝 `backend/.env.example` →
`backend/.env`。

| 变量 | 默认 | 用途 |
|-----|---------|---------|
| `LAX_DATABASE_URL` | `sqlite:///./data/little_alphaxiv.db` | SQLite 文件（相对路径基于 `backend/` 解析）。默认 `backend/data/little_alphaxiv.db`；Docker 下为 `sqlite:////app/data/little_alphaxiv.db`。密钥与重置日志与 DB 同目录。 |
| `LAX_SECRET_KEY` | *(自动生成)* | 用于加密存储的 API key + 签名会话 cookie 的 Fernet key。首次运行自动生成并写入 `backend/data/.lax_secret_key`（本地）或 `./data/.lax_secret_key`（Docker）——DB、PDF 缓存、密钥、重置日志均集中在同一数据目录。**务必保密——丢失它会让所有加密 key + 会话全部失效。** |
...
| `LAX_PDF_CACHE` | `backend/data/pdf_cache` | PDF 磁盘缓存目录（按内容寻址、全局、非敏感）。Docker 下为 `/app/data/pdf_cache`。 |
...
├── docker-compose.yml        # 一键自托管运行
└── docker/entrypoint.sh      # 自动生成并持久化 LAX_SECRET_KEY
```
...
`./data/.lax_secret_key`（Docker）或 `backend/data/.lax_secret_key`（本地）。
</details>
