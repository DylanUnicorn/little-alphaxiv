...
```bash
chmod +x LittleAlphaxiv-<version>-<arch>.run
./LittleAlphaxiv-<version>-<arch>.run
```

首次启动会自动打开浏览器。程序文件放在 `~/.cache/little-alphaxiv/`，你的
数据库、PDF 缓存、密钥和设置保存在 `$XDG_DATA_HOME/little-alphaxiv` 或
`~/.local/share/little-alphaxiv`。如需换端口：

```bash
LAX_PORT=8080 ./LittleAlphaxiv-<version>-<arch>.run
```
