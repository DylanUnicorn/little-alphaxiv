...
RUNNER="$OUT_ROOT/${APP_NAME}-${VERSION}-$(uname -m).run"
...
export LD_LIBRARY_PATH="$PYTHONHOME_DIR/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
...
# Bundle libpython if the interpreter is dynamically linked. Most distro
# pythons (apt/dnf-installed) link libpython3.x.so.1.0; without it the bundled
# binary fails to start on a target machine that lacks the matching libpython.
# `cp -L` follows the soname symlink and copies the real file under its soname,
# which is what the dynamic loader looks up. Statically-linked pythons (no
# libpython in `ldd` output) skip this step cleanly.
echo "[build-linux-run] Bundling libpython (if dynamically linked)"
mkdir -p "$APP_DIR/python/lib"
ldd "$PY_EXE" 2>/dev/null | grep -oE '/[^ ]*libpython[^ ]*' | sort -u | while read -r lib; do
  [ -n "$lib" ] && [ -f "$lib" ] && cp -L "$lib" "$APP_DIR/python/lib/"
done
